...
static bool	map_init(t_map *map)
{
	...
	if (map->map[0] == NULL)
		return (free(map->map), true);
	map->map[1] = ft_strdup("100101");
	if (map->map[1] == NULL)
		return (free(map->map[0]), free(map->map), true);
	map->map[2] = ft_strdup("101001");
	if (map->map[2] == NULL)
		return (free(map->map[0]), free(map->map[1]), free(map->map), true);
	map->map[3] = ft_strdup("1100N1");
	if (map->map[3] == NULL)
		return (free(map->map[0]), free(map->map[1]), \
			free(map->map[2]), free(map->map), true);
	map->map[4] = ft_strdup("111111");
	if (map->map[4] == NULL)
		return (free(map->map[0]), free(map->map[1]), \
			free(map->map[2]), free(map->map[3]), free(map->map), true);
	map->map[5] = NULL;
	map->max_width = 6;
	map->max_height = 5;
	return (false);
